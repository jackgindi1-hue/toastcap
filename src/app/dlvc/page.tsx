...
function DLVCPageContent() {
  ...
  useEffect(() => {
    const validateToken = async () => {
      ...
      try {
        const res = await fetch(`/api/tokens?token=${currentToken}&page=dlvc`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Progress token status to completed
        if (token) {
          await fetch('/api/tokens', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ token, page: 'dlvc' }),
          });
