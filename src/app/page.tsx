...
function InteractiveBarChart() {
  ...
  return (
    <div className="max-w-5xl mx-auto flex flex-col items-center">
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl p-6 md:p-8 shadow-lg border border-gray-100">
        {/* Bars Container */}
        <div className="flex items-end justify-between gap-3 md:gap-6 mb-4" style={{ height: '340px' }}>
          {chartData.map((d, i) => {
            const heightPercent = (d.sales / maxSales) * 100;
            const paymentHeightPercent = (d.payment / maxSales) * 100;
            const isSelected = selectedBar === i;
            return (
              <div
                key={i}
                className="flex-1 flex items-end justify-center gap-1 cursor-pointer group"
                style={{ height: '100%' }}
              ...
              >
                ...
                <div
                  className={`relative rounded-t-lg transition-all duration-300 ${
                    isSelected ? 'shadow-lg scale-105' : 'group-hover:opacity-80'
                  }`}
                  style={{
                    width: viewMode === 'daily' ? '60%' : '55%',
                    height: `${heightPercent}%`,
                    minHeight: '20px',
                    background: isSelected
                      ? 'linear-gradient(180deg, #FF8F6B 0%, #FF6B35 100%)'
                      : 'linear-gradient(180deg, #FFB49A 0%, #FF9B7A 100%)',
                    boxShadow: isSelected ? '0 4px 20px rgba(255, 107, 53, 0.4)' : '0 2px 8px rgba(0,0,0,0.1)',
                  }}
                >
                  {/* Plus icon on bar */}
                  <div className={`absolute -top-4 left-1/2 -translate-x-1/2 w-7 h-7 rounded-full bg-[#FF6B35] flex items-center justify-center text-white text-sm font-bold shadow-md transition-all duration-200 ${isSelected ? 'opacity-100 scale-110' : 'opacity-0 group-hover:opacity-100'}`}>
                    +
                  </div>
                </div>
