...
// Types
export type LeadStage = 'quote' | 'application' | 'dlvc' | 'approved' | 'funded';
...
export interface Lead {
  ...
  dripStyle?: 'original' | 'v2'; // Email style: original (gray wrapper) or v2 (website style)
  dripStep?: number;
  ...
  approvedTerm?: string; // e.g., "12 months"
  approvedRepaymentType?: 'daily' | 'weekly';
  approvedRepaymentAmount?: string; // e.g., "$285/day"
}
...
function rowToLead(row: any): Lead {
  return {
    ...
    dripStyle: row.drip_style || undefined,
...
function leadToRow(lead: Partial<Lead>) {
  ...
  if (lead.dripStyle !== undefined) row.drip_style = lead.dripStyle;
...
// Updated to 10 steps over 18 days
export const DRIP_SCHEDULE_DAYS = [0, 1, 2, 4, 6, 7, 9, 12, 15, 18];
export const DRIP_SEND_HOURS = [9, 13, 9, 13, 9, 13, 9, 13, 9, 13];
...
export async function bulkStartDripCampaign(
  ...
  if (eligibleIds.length > 0) {
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        drip_style: style, // Store email style (original or v2)
  ...
  console.log(`✅ Bulk drip (${style}): ${eligibleIds.length} started, ${leadIds.length - eligibleIds.length} skipped`);
